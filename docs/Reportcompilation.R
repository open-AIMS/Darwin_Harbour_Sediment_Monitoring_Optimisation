...
## ---- STAGE 2
{
    ...
    doc <- doc %>%
        ...
        body_add_docx('sediment_monitoring_word.docx') %>%
    ...
    doc %>% print(target = 'Sediment Monitoring Optimisation.docx')
    ## ----end
}
## ----end
...
##         body_add_break() %>%
##         body_add_docx('sediment_monitoring_word.docx') %>%
##         set_doc_properties(creator = "Murray Logan", created = Sys.time()) %>%
##         footers_replace_all_text(old_value = 'Marie Roman',
##                                  new_value = metadata$TITLE,
##                                  only_at_cursor = FALSE, fixed = FALSE, warn=FALSE)
