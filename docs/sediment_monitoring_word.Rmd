...
The data used in these analyses were provided by Lynda Radke via email
and are summarised in Table \@ref(tab:dataSource).  For the purpose of
these analyses the East, Middle and West Arms are combined into a
single Inner Habour.

</br>

The physiochemical data comprise both raw and _normalised_ versions of
Copper (Cu), Lead (Pb) and Zinc (Zn) from a collection of designated
(fixed) and random (free) sites.  The fixed sites serve multiple
purposes (i.e. they also have a regulatory roll) and thus must be
preserved throughout the optimisation analyses.  Contrastingly, the
free sites are dedicated to the sediment monitoring program and thus
are all potential candidates for optimisation.
...
were computed against either Aluminium (Al) or Iron (Fe) conditional
on the Iron to Aluminium ratio:
...
is called **Inverse distance weighting**.  Inverse distance weighting
[IDW: @Cressie-1993] is a deterministic smoothing method in which the
...
interpoliation methods (such as kriging or stochastic partial
differential equations), most of these alternative proceedures either
require human tuning for each model or else are too computationally
expensive to be feasible in in the current context.  For example,
...
nearest water infrastructure feature.  These distances are illustrated
...
measures in the future, if that spike occurs in a area for which the
...
sampling configuration that itself heavily biased sampling sites away
...
asp_rat <- dim(my_pic)[2]/dim(my_pic)[1] #find our aspect ratio
```
```{r configINRAWwtdPart1, echo=FALSE, fig.width=6, fig.asp = asp_rat, message = FALSE, fig.cap = 'Best spatial configurations of weighted fixed (dark blue) and free (maroon) sites associated with increasing numbers of free sites.  Open yellow symbols represent the unselected sites from the full saturated configuration.', fig.id = "configINRAWwtdPart1", fig.cap.style = "Image Caption"}
...
asp_rat <- dim(my_pic)[2]/dim(my_pic)[1] #find our aspect ratio
```
```{r configINRAWwtdPart2, echo=FALSE, fig.width=6, fig.asp = asp_rat, message = FALSE, fig.cap = 'Best spatial configurations of weighted fixed (dark blue) and free (maroon) sites associated with increasing numbers of free sites.  Open yellow symbols represent the unselected sites from the full saturated configuration.', fig.id = "configINRAWwtdPart2", fig.cap.style = "Image Caption"}
...
asp_rat <- dim(my_pic)[2]/dim(my_pic)[1] #find our aspect ratio
```
```{r configINNORMALISEDwtdPart1, echo=FALSE, fig.width=6, fig.asp = asp_rat, message = FALSE, fig.cap = 'Best spatial configurations of weighted fixed (dark blue) and free (maroon) sites associated with increasing numbers of free sites.  Open yellow symbols represent the unselected sites from the full saturated configuration.', fig.id = "configINNORMALISEDwtdPart1", fig.cap.style = "Image Caption"}
...
asp_rat <- dim(my_pic)[2]/dim(my_pic)[1] #find our aspect ratio
```
```{r configOHNORMALISEDwtdPart1, echo=FALSE, fig.width=6, fig.asp = asp_rat, message = FALSE, fig.cap = 'Best spatial configurations of weighted fixed (dark blue) and free (maroon) sites associated with increasing numbers of free sites.  Open yellow symbols represent the unselected sites from the full saturated configuration.', fig.id = "configOHNORMALISEDwtdPart1", fig.cap.style = "Image Caption"}
...
prolonged perior (e.g. 5 or 10 years) might be high.
...
[pandoc](pandoc.org) universal format converted.  On first openning in
